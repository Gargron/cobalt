...
module HttpSignatureVerification
  ...
  def signed_request_account
    ...
    if account.key.public_key.verify(OpenSSL::Digest::SHA256.new, signature, compare_signed_string)
  ...
  def account_from_key_id(key_id)
    return if TagManager.local_uri?(key_id)

    account   = TagManager.uri_to_resource(key_id, Account)
    account ||= FetchRemoteKeyService.new.call(key_id, id: false)
    account
  end
end
