...
class Request
  ...
  def signature
    ...
    signature = Base64.strict_encode64(@account.key.sign(OpenSSL::Digest::SHA256.new, signed_string))
  ...
  def user_agent
    @user_agent ||= "#{HTTP::Request::USER_AGENT} (Cobalt/Alpha; +#{root_url})"
  end

  def key_id
    [TagManager.uri_for(@account), '#main-key'].join
  end
